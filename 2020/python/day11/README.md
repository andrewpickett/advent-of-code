...
I really dislike manipulating multi-dimensional arrays...unless I'm using a mathematical language.
I realize Python has a lot of math libraries and is one of the biggest "math" languages right now, but
I'm using "vanilla" Python, and I'm just not yet experienced enough with the language to know how to
model and manipulate these structures quite as efficiently.

### Part 1
> _How many seats end up occupied?_

Anyways...I took a pretty straightforward approach to this puzzle and just looped over every element,
created a list of each of the immediate neighbors (taking into account the borders!) and then counting
the number of occupied seats. It isn't fast...it isn't graceful...but it got the job done.

I then spent a while the next day trying to come up with a more creative and optimized approach to it...
but even with a number of changes and updates, I wasn't achieving the speed I wanted. I could have spent
a lot longer in it, as I believe I know WHERE the issue lies (creating copies of the input every time),
I don't really feel like putting any more thought or energy into this, so it is what it is. It works.

### Part 2
> _Given the new visibility method and the rule change for occupied seats becoming empty, once equilibrium
is reached, how many seats end up occupied?_

Really the only change here, was when getting the neighbors, I had to loop over every neighbor in the row
until I get to the first blank (`.`). Then use THAT as the neighbor in the algorithm instead of the
immediate neighbor. Nothing else really changed, but it definitely slowed the code down...so much so that
at first I didn't think it would finish, but it finally did, and we'll leave it at that.

# Results

|    | Answer     | Attempts  | Exec. Time (ms) | Solve Time (HH:mm:ss) | Rank |
| ------ |-----------:| ---------:| -------------------:| ----:| ----:|
| **Part One**  | 2319  | 1  | 2077  | 00:43:21  | 3767  |
| **Part Two**  | 2117  | 1  | 17833  | 00:31:00  | 3690  |
...
predictable and always follow a simple set of rules. All decisions are based on the number of occupied seats adjacent to a given seat (one of the eight positions immediately up, down, left, right, or diagonal from the seat). The following rules are applied to every seat simultaneously:
